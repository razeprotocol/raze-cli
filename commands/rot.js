...
Response:`;
  ...
  parseTextResponse(text) {
    ...
    // System info request
    if (lowerText.includes("system info") || lowerText.includes("system information") || lowerText.includes("show me system")) {
      actions.push({
    ...
    // Chrome opening
    if (lowerText.includes("open chrome") || lowerText.includes("launch chrome") || lowerText.includes("start chrome")) {
      actions.push({
        action: "OPEN_CHROME",
        parameters: {},
    ...
    // VS Code
    if (lowerText.includes("open vscode") || lowerText.includes("open vs code") || lowerText.includes("code")) {
      actions.push({
        action: "OPEN_VSCODE",
        parameters: { path: "." },
    ...
    if (lowerText.includes("create file") || lowerText.includes("make file")) {
      const fileMatch = text.match(/(?:create|make)\s+(?:file|a file)\s+(?:called\s+)?["\']?([^"\']+)["\']?/i);
      if (fileMatch) {
        actions.push({
          action: "CREATE_FILE",
    ...
    // Folder creation
    if (lowerText.includes("create folder") || lowerText.includes("make folder") || lowerText.includes("create directory")) {
      const folderMatch = text.match(/(?:create|make)\s+(?:folder|directory)\s+(?:called\s+)?["\']?([^"\']+)["\']?/i);
      if (folderMatch) {
        actions.push({
  ...
  async executeAction(action) {
    ...
    try {
      switch (action.action) {
        ...
        case "CREATE_FILE":
          await this.createFile(action.parameters.path, action.parameters.content);
          break;
        ...
        case "RUN_COMMAND":
          await this.runCommand(action.parameters.command, action.parameters.cwd);
          break;
        ...
        case "SEND_NOTIFICATION":
          await this.sendNotification(action.parameters.title, action.parameters.message);
          break;
...
export default function registerRot(program) {
  program
    .command("rot")
    .description("🤖 ROT Agent - Your intelligent system assistant powered by Gemini AI")
    .argument("[request...]", "What you want ROT to do (e.g., 'open chrome and search web3 grants')")
    ...
    .action(async (requestParts, opts) => {
      ...
      let request = requestParts && requestParts.length ? requestParts.join(" ") : undefined;
      ...
      try {
        ...
        if (successCount < actions.length) {
          console.log(chalk.yellow("Some actions failed. Check the output above for details."));
        }
